...
exports.OnboardingProgressController = void 0;
...
class OnboardingProgressController {
    ...
    async initializeProgress(req, res) {
        try {
            ...
            const existingProgress = await onboardingProgress_1.OnboardingProgress.findOne({ companyId: companyObjectId });
            ...
            (0, onboardingProgressUtils_1.applyComingSoonFlags)(initialProgress.phases);
    ...
    async ensureConsistency(progress) {
        let modified = false;
        // 0. Migrate to new step structure:
        //    Phase 2: steps 3,4,5 only (Reporting step removed)
        //    Phase 3: 7=KB, 8=REP Onboarding, 9=Call Script, 10=Session Planning
        const { modified: stepsMigrated, newCompletedSteps } = (0, onboardingProgressUtils_1.migrateToNewStepStructure)(progress.phases, progress.completedSteps ?? []);
        if (stepsMigrated) {
        ...
        for (const stepId of progress.completedSteps) {
            if (!computedCompletedSteps.includes(stepId)) {
                ...
                const phase = progress.phases.find((p) => p.steps.some((s) => s.id === stepId));
                if (phase) {
        ...
        const phase1 = progress.phases.find((p) => p.id === 1);
        ...
        if (step1Done && phase1) {
            ...
            if (phase1.status !== 'completed' || progress.currentPhase < 2) {
                (0, onboardingProgressUtils_1.advanceAfterProfileCreated)(progress.phases);
        ...
        for (let i = 0; i < progress.phases.length; i++) {
            ...
            const allStepsCompleted = (0, onboardingProgressUtils_1.isPhaseComplete)(phase);
            const activeSteps = phase.steps.filter(onboardingProgressUtils_1.isActiveStep);
            ...
            if (targetStatus === 'completed' && nextPhase) {
                ...
                const firstActivePendingStep = nextPhase.steps.find((s) => (0, onboardingProgressUtils_1.isActiveStep)(s) && s.status === 'pending');
                const hasAnyStepInProgressOrCompleted = nextPhase.steps.some((s) => s.status === 'in_progress' || s.status === 'completed');
        ...
        const currentActivePhase = progress.phases.find((p) => p.status === 'in_progress');
    ...
    async updateStepProgress(req, res) {
        try {
            ...
            if (status === 'completed') {
                ...
                const nextStep = phase.steps
                    ...
                    .find((s) => (0, onboardingProgressUtils_1.isActiveStep)(s) && s.status !== 'completed');
                if (nextStep) {
                    ...
                    if (nextPhase) {
                        ...
                        const firstAvailableStep = nextPhase.steps.find((s) => (0, onboardingProgressUtils_1.isActiveStep)(s) && s.status !== 'completed');
    ...
    async updateCurrentPhase(req, res) {
        try {
            const { companyId } = req.params;
    ...
    async resetProgress(req, res) {
        try {
            const { companyId } = req.params;
            const companyObjectId = new mongoose_1.Types.ObjectId(companyId);
            await onboardingProgress_1.OnboardingProgress.findOneAndDelete({ companyId: companyObjectId });
    ...
    async getProgressByUserId(req, res) {
        try {
            const { userId } = req.params;
            console.log('userId reçu:', userId);
            // Trouver la company associée au userId
            const company = await CompanyModel_1.CompanyModel.findOne({ userId: new mongoose_2.default.Types.ObjectId(userId) });
            if (!company) {
            ...
            const progress = await onboardingProgress_1.OnboardingProgress.findOne({
    ...
    async completeLastPhaseAndStep(req, res) {
        try {
            const { companyId } = req.params;
...
exports.OnboardingProgressController = OnboardingProgressController;
